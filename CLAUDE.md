...
- **`config.py`** — loads `categories.json` (extension → category mapping, plus the `subcategories` → subfolder rules) into module-level globals (`CATEGORY_EXTENSIONS`, `CATEGORY_ORDER`, `MISC_CATEGORY`, `SUBCATEGORY_EXTENSIONS`, `SUBFOLDERS_ENABLED`); `reload_categories()` lets callers hot-reload after editing the JSON. Also resolves standard user folders (Downloads, Desktop, etc.) and reads env vars (`CLUTTERCTRL_LOG_DIR`, `CLUTTERCTRL_SUBFOLDERS`, `WATCHDOG_DEBOUNCE_SECONDS`, `MAX_LOG_FILES`).
- **`cleaner.py`** — the core engine: `process_directory()` (top-level scan) and `deep_scan_directory()` (recursive, also cleans up empty folders afterward via `cleanup_empty_dirs`/`preview_empty_dirs`). Both accept `dry_run`, `quiet`, `subfolders`, and an optional `event_callback(event_dict)` for streaming progress events (this callback hook exists for a UI/websocket consumer even though no such consumer currently ships in this repo — preserve it when refactoring). `subfolders=None` means "use the configured default" (`resolve_subfolders`), `True`/`False` force the two-level or flat layout. Category folders (`is_category_folder`) are never descended into or deleted, and nothing *under* a category folder is either (`is_inside_category_folder`) — that's what keeps the generated subfolders from being re-sorted or swept away as empty.
...
- **`helpers.py`** — small stateless utilities: `get_category`/`get_subcategory`/`get_dest` (extension → category / subfolder / destination path), `extension_folder_name` (fallback subfolder for extensions with no rule, `.xyz` → `XYZ`), `make_unique` (collision-safe renaming, e.g. `photo_1.png`), `format_bytes`, `ensure_dir`.
- **`categories.json`** — the actual extension-to-category data (`{"categories": {...}, "misc_category": "Misc", "subfolders_enabled": false, "subcategories": {"Documents": {"Spreadsheets": [".xlsx", ...]}}}`), packaged via `[tool.setuptools.package-data]` so it ships with the wheel. Edit this file, not hardcoded category lists in Python, to add/change extension or subfolder mappings.
...
- Deep scan and the watcher both use `is_category_folder` / `is_inside_category_folder` (from `cleaner.py`) to avoid ever recursing into, re-sorting, or deleting the category folders — and the per-extension subfolders inside them — that they themselves create.
- Subfolder placement must stay a pure function of the extension (`helpers.get_subcategory`), and the *full* source and destination paths must keep going into the run log, or undo cannot put a file back where it came from.
...
- `tests/conftest.py` redirects `CLUTTERCTRL_LOG_DIR` to a temp directory so the suite never writes real run logs into `clutterctrl/logs/`. Any new test module gets that for free; don't import `clutterctrl` before conftest runs.
- There is still no `rules` *editing* command — adding an extension or a subfolder group means hand-editing `categories.json` (`config.reload_categories()` exists for picking the change up without a restart).
