...
- **`main.py`** — argparse CLI + interactive terminal menu. Owns all `print()`/color output. Subcommands: `clean`, `scan` (dry-run), `watch`, `history`, `undo`, `stats`, `rules`. Also supports legacy flag-based invocation (`--target`, `--clean`, `--dry-run`, etc.) for backwards compatibility, and falls back to `interactive_menu()` when invoked with no args. Note the sys.path shim at the top of the file that lets it run both as `python clutterctrl/main.py` (direct script) and `python -m clutterctrl.main`.
